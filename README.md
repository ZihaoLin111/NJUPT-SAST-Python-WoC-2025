...
DnCNN 模型分为三部分，默认总层数nums_layers=17。第一部分，输入层，由Conv+ReLU组成，卷积核大小为3\*3，padding=1，映射至64通道；第二部分，中间层，其中包含nums_layer-2个重复的小层，每小层由Conv+BN+ReLU组成；第三部分，Conv做为输出层，重新映射回3通道

DnCNN预测的是图片的噪声，最后输出结果为“含噪声的原图” - “模型预测的噪声”

![DnCNN结构图](/MTL/charts/DnCNN.JPG)
...
![ResNet18结构图](/MTL/charts/ResNet18.JPG)
...
#### 4.1.消融实验
最后我又将预训练的DnCNN在CIFAR10-C的Gaussian Noise上直接进行测试以及使用多任务学习方法训练未经预训练的DnCNN在CIFAR10-C的Gaussian Noise上进行测试，结果如下表所示
|   Severity   |  SIDD + DnCNN Accuracy   |  DnCNN + MTL Accuracy   |
| ---- | ---- | ---- |
|   1   |   56.50%   |   71.80%   | 
|   2   |   45.70%   |   68.70%   |
|   3   |   35.10%   |   63.50%   |
...
测试结果表明，直接使用预训练的DnCNN在CIFAR10-C的Gaussian Noise上进行测试的准确率较低；使用多任务学习方法训练未经预训练的DnCNN在CIFAR10-C的Gaussian Noise上进行测试的准确率有小幅度提高，但仍然远低于使用多任务学习方法微调预训练的DnCNN在CIFAR10-C的Gaussian Noise上进行测试的准确率。
